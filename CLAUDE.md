...
responses with **`<emoji> DN <context> · <Stage>`** before proceeding (where `<emoji>` is the
...
2. Tag your response with **`<emoji> DN <context> · <Stage>`** to confirm the framework is active and show which stage you are in.
...
**`<emoji> DN <context> · <Stage>`** tag. This serves as a built-in signal:
...
Examples:
> "Ready to commit all of this? **`🤓 DN 2.10.0 · Feature`**"
> "Shall I push this to origin? **`🤓 DN 2.10.0 · Feature`**"
> "Ready to reset `master` to match `staging`? **`⚠️ DN 2.10.0 · Production`**"
