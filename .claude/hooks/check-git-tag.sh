...
if echo "$COMMAND" | grep -q 'staging/active'; then
  if echo "$COMMAND" | grep -q '\-d'; then
    EXTRA="\n\n🏷️ This CLEARS the staging claim. Only do this after master has been reset and deployment is confirmed healthy."
  else
    EXTRA="\n\n🏷️ This CLAIMS staging. Ensure staging contention checks have passed. Notify the team in Slack after this."
  fi
fi
